...
47. [DONE][P1] Add pause duration metrics to live feedback and summaries
Acceptance Criteria: Pause detection tracks average pause duration for the current session; the live feedback panel displays pause count and average pause duration (seconds) while recording; analysis summaries persist average pause duration alongside pause count and threshold; recordings list shows pause count and average pause duration when a summary exists; unit tests cover pause duration tracking, UI formatting, and summary persistence/loading with local-only data; no network usage is introduced.
48. [READY][P1] Allow selecting a preferred microphone input device
Acceptance Criteria: Settings list of available microphone inputs allows selecting a preferred device; the selection persists locally and becomes the active input for capture; if the selected device becomes unavailable, capture falls back to the system default and surfaces a clear local-only status message; the selection list updates on device connect/disconnect; unit tests cover persistence, active device selection, fallback behavior, and list refresh with mocked providers; no network usage is introduced.
